...
# TODO: Implement these another time
...
def initialize_images():
    ...
    for file in folder.get_image_list():
        ...
        # log.write_info(Info.EXECUTE, None, logger, None)


def initialize_cropped_images():
    image_list = []
    for image in images:
        cropped_image = Cropped(image.get_name(), image.get_index())


def image_save(crop_image, index, comic_image):
    """
    Writes crop_image to the cropped directory.
    ...
    NONE
    """
    file_name = comic_image.get_name()
    cropped_image_name = str(os.path.join(folder.get_cropped_path(), "cropped_{}_".format(str(index)) + file_name))
    print(cropped_image_name)
    cv2.imwrite(cropped_image_name, crop_image)
